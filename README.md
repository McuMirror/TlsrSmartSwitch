...
И ошибка с выбором GPIO кнопки может потребовать такой разборки для очистки Flash и перепрошивки.
Установки GPIO необходимо создать для первой версии обновления оригинальной прошивки через ZigbeeOTA правильно, без ошибок для кнопки.
Иначе, если GPIO кнопки не совпадет, тогда могут быть проблемы со сбросом устройства для регистрации в сети.
В дальнейших обновлениях через ZigbeeOTA можно использовать любую новую версию, не смотря на то, какие там назначены GPIO. Будут назначены ранее сохраненные в этом устройстве.
